...
describe('parseIncoming', () => {
    ...
    test('accepts a session message', () => {
        const message = parseIncoming({ sessionId: 'abc' });
        expect(message).toEqual({ kind: 'session-message', sessionId: 'abc' });
    });
    ...
    test('accepts a tensor message and keeps only name and tensor', () => {
        ...
        const message = parseIncoming({ name: 'first', tensor, extra: 'ignored' });
        expect(message).toEqual({ kind: 'tensor-message', name: 'first', tensor });
    });
    ...
    test('reports each missing tensor field', () => {
        const fields = ['dtype', 'shape', 'stride', 'size_bytes', 'encoding', 'blob'] as const;
