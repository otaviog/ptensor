// The socket the debuggee pushes tensors into.
//
// The extension listens on an ephemeral loopback port and hands the address to
// the debugger, which calls `p10::tlog::log_to(<address>, ...)` in the stopped
// frame. The tensor then travels as bytes over TCP instead of through the
// debugger's expression printer, which is what used to cap the size: LLDB
// truncates its string summaries, and reading a large one back a chunk at a
// time is slow enough to feel broken.
//
// The framing and message parsing are ptensor-tlog's, shared with the desktop
// app; this file is only the node:net transport around them.
...
export interface TensorFeedOptions {
    ...
    onTensor: TensorSink;
    /** Where the feed reports trouble; the extension routes this to its channel. */
