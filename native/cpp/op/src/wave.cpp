...
generate_sine_wave(size_t num_samples, Dtype type, const SineWaveOptions& options, Tensor& output) {
    ...
    const double freq = options.frequency();
    const double sample_rate = options.sample_rate();
    ...
    const double phase = options.phaseRadians();

    const size_t period_samples = static_cast<size_t>(
        options.period().has_value() ? options.period().value() * sample_rate : (sample_rate / freq)
    );
    if (period_samples == 0) {
    ...
    output.visit([&](auto span) {
        ...
        const scalar_t phase_increment = static_cast<scalar_t>(2.0 * M_PI / period_samples);
