...
class TensorExtents {
  public:
    ...
    TensorExtents(TensorExtents&& other) noexcept : extent_(other.extent_), dims_(other.dims_) {
        other.dims_ = 0;
    ...
    P10Result<TensorExtents> permute_extents(const std::span<const size_t>& perm) const {
        ...
        for (size_t i = 0; i < dims_; i++) {
            size_t const perm_index = perm[i];
  ...
  protected:
    TensorExtents(std::span<const int64_t> shape) : dims_(shape.size()) {
        std::ranges::copy(shape, extent_.begin());
    }
