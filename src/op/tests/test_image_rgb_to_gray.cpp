...
TEST_CASE("RGB to gray: Mixed color values", "[image_rgb_to_gray]") {
    ...
    REQUIRE(gray_span[1][1] == Approx(143).margin(1));
}
...
TEST_CASE("RGB to gray: Luminosity formula accuracy", "[image_rgb_to_gray]") {
    ...
    for (const auto& test : test_cases) {
        ...
        REQUIRE(gray_span[0][0] == Approx(expected).margin(1));
    }
}
...
TEST_CASE("RGB to gray: Gradient pattern", "[image_rgb_to_gray]") {
    ...
    REQUIRE(gray_span[0][0] == Approx(expected_tl).margin(1));

    float const expected_br = 0.21f * 255 + 0.72f * 255 + 0.07f * 128;
    REQUIRE(gray_span[15][15] == Approx(expected_br).margin(1));
}
