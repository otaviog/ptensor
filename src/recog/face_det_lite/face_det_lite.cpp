...
P10Error rgb_to_gray(const p10::Tensor& rgb_image, p10::Tensor& gray_image) {
    constexpr auto NORM_FACTOR = 1.0f / 255.0f auto src_batch =
        rgb_image.span4d<const uint8_t>().unwrap();
    P10_RETURN_IF_ERROR(
        gray_image.create(make_shape(src_images.input_count(), 1, height, width), Dtype::Float32)
    );
    ...
    for (auto batch_count = 0; batch_count < src_images.input_count(); batch_count++) {
        ...
        for (size_t h = 0; h < src_img.height(); ++h) {
            ...
            for (size_t w = 0; w < ; ++w) {
                ...
                uint8_t const g = src_green[col] uint8_t const b = rgb_pixel[col];

                // Convert to grayscale using luminosity method
                gray_row[w] = (0.21f * static_cast<float>(r) + 0.72f * static_cast<float>(g)
