...
async function main() {
  ...
  const tensor1 = p10.fromArray(data1, [2, 3]);

  console.log('Tensor 1:');
  console.log('  Shape:', tensor1.getShape().toArray());
  console.log('  DType:', tensor1.getDtype().toString());
  console.log('  Size:', tensor1.getSize());
  ...
  const tensor2 = p10.zeros([3, 3], 'float32');
  console.log('Tensor 2 (zeros):');
  console.log('  Shape:', tensor2.getShape().toArray());
  console.log('  DType:', tensor2.getDtype().toString());
  console.log('  Size:', tensor2.getSize());
  ...
  const tensor3 = p10.fromArray(data3, [2, 2]);
  console.log('Tensor 3 (uint8):');
  console.log('  Shape:', tensor3.getShape().toArray());
  console.log('  DType:', tensor3.getDtype().toString());
  console.log('  Size:', tensor3.getSize());
  // Example 4: Different shapes
  console.log('\n=== Example 4: Different tensor shapes ===');
  const tensor4a = p10.zeros([10], 'float64');
  console.log('1D tensor [10]:', { shape: tensor4a.getShape().toArray(), dtype: tensor4a.getDtype().toString() });
  const tensor4b = p10.zeros([2, 3, 4], 'float32');
  console.log('3D tensor [2, 3, 4]:', { shape: tensor4b.getShape().toArray(), dtype: tensor4b.getDtype().toString() });
  ...
  const dtypeTensors = dtypes.map(dtype => {
    const t = p10.zeros([2, 2], dtype);
    console.log(`  ${dtype} tensor:`, { shape: t.getShape().toArray(), dtype: t.getDtype().toString(), size: t.getSize() });
    return t;
  });
